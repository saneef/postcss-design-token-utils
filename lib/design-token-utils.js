...
function generateUtilityClassRules(
  ...
  const rule = new Rule({ selector: selector(classObject), source });
  properties.forEach((prop) => {
    rule.append(
      new Declaration({ prop, value: /** @type {string} */ (value), source }),
    );
  });
  return rule;
}
...
function createCustomProperties(
   ...
   * @param      {string=}  rootId    The id of the token set
   * @return     {Array<TokenObject>}
   */
  function createTokenObject(object, rootId, parentId) {
    return Object.keys(object).reduce(
      ...
      (acc, k) => {
        ...
        const groupKey = rootId === undefined ? k : rootId;
        ...
        return [...acc, ...createTokenObject(object[k], groupKey, prop)];
      },
      [],
    );
  }
  ...
  return options.flatMap(
    ({ id, property, prefix, responsiveVariants = false }) => {
      return tokenObjects
        ...
        .map(({ prop, value, name, parentId }) => {
          const selectorPrefix = prefix ?? id;
          const selectorName = `${parentId}.${name}`.replace(id, "");
          const selectorBase = kebabCase(`${selectorPrefix}.${selectorName}`);
          return {
            selectorBase,
            prop: property,
            value: `var(${prop})`,
